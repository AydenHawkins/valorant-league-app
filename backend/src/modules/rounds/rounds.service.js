...
const createRound = async (data) => {
    ...
    return await roundsRepository.create({ matchId, roundNumber, result, winningTeam });
};

const updateRound = async (id, data) => {
    const { matchId, roundNumber, result, winningTeam } = data;
    return await roundsRepository.update(id, { matchId, roundNumber, result, winningTeam });
};
